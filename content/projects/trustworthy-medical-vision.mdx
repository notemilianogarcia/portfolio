...
Binary pneumonia classification on the **Kaggle Chest X-Ray Images dataset** with a rigorous reliability pipeline. The system answers a specific question: **when should predictions be trusted?**

**Core Models & Metrics**
- DenseNet-121 baseline: **0.9576 AUROC**, 0.8990 accuracy, 0.9240 F1
- ResNet-50 comparator: **0.9713 AUROC**, 0.8798 accuracy (complementary strengths)
- Deterministic train/val/test splits with patient-level separation

**Reliability Stack**
- **Calibration**: Temperature scaling with adaptive binning; reliability diagrams quantify confidence alignment
- **Uncertainty**: MC Dropout with strategic placement yielding nuanced uncertainty signals vs. raw confidence
- **Explainability**: Grad-CAM visual explanations validated against untrained model baselines (sanity checks)
- **Explanation Stability**: Robustness under perturbations (±5° rotation, brightness shifts) via Spearman correlation and spatial metrics
- **Failure Audit**: Systematic categorization of 63 misclassifications into 4 distinct modes (clinical ambiguity, annotation noise, model blindness, spurious features)
- **Ensemble & Deferral**: 3-model ensemble with learned abstention policies; **36.63% estimated cost reduction** in failure scenarios
- **Interactive Demo**: Next.js web application for per-case and system-level exploration without local setup

**The Key Insight**: High AUROC ≠ trustworthy. Calibration adjustments don't change accuracy but reveal reliability gaps. Uncertainty and explanation stability distinguish confident-but-fragile predictions from robust ones. Cross-model disagreement (32 of 624 cases) becomes valuable signal, not a problem.
